...
$stmt = $conn->prepare("UPDATE leitores
                        SET nomeLeitor = ?, funcaoLeitor = ?, localLeitor = ?, contatoLeitor = ?
                        WHERE codigoLeitor = ?");

// Vincule os parâmetros com os valores (todos os parâmetros são strings -> "sssss")
$stmt->bind_param("sssss", $nomeLeitor, $funcaoLeitor, $localLeitor, $contatoLeitor, $codigoLeitor);

// Execute a consulta
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
		echo json_encode(['success' => true, 'message' => 'Cadastro do leitor alterado com sucesso!']);
    } else {
