...
$stmt = $conn->prepare("UPDATE emprestimos
                        SET codigoEmprestimo = ?, codigoLivro = ?, nomeLivro = ?, codigoLeitor = ?, nomeLeitor = ?, dataRetirada = ?, dataDevolucao = ?
						WHERE codigoEmprestimo = ?");

// Vincule os parâmetros com os valores (s -> string, i -> integer)
$stmt->bind_param("ssssssss", $codigoEmprestimo, $codigoLivro, $nomeLivro, $codigoLeitor, $nomeLeitor, $dataRetirada, $dataDevolucao, $codigoEmprestimo);
